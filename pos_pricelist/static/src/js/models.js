...
function pos_pricelist_models(instance, module) {
    ...
    module.Order = module.Order.extend({
        ...
        addProduct: function (product, options) {
            ...
            var self = this;
            var found = false;
            var pos_discount=this.getPosDiscount(product, line);
            if (pos_discount>0)
            {
                line.set_discount(pos_discount)
            }
        ...
        getPosDiscount: function (product, line){
            ...
            if (partner && partner.property_product_pricelist) {
                var version = this.pos.pricelist_engine.find_valid_pricelist_version(this.pos.db, partner.property_product_pricelist[0]); 
            } else {
            ...
            if (product.categ_id) {
                ...
                categ_ids = categ_ids.concat(
                    this.pos.db.product_category_ancestors[product.categ_id[0]]
                );
            }

            // find items
            var items = [], i, len;
            for (i = 0, len = this.pos.db.pricelist_item_sorted.length; i < len; i++) {
                var item = this.pos.db.pricelist_item_sorted[i];
    ...
    module.Orderline = module.Orderline.extend({
        ...
        get_product: function(){
            return OrderlineParent.prototype.get_product.apply(
                this, arguments
            );
        },
