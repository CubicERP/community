                ...
                if r['EmisionTipo'] == 'CAE':
                    ...
                    if inv_ids and qi.update_invoices and len(inv_ids) == 1:
                        ...
                        for iva in r['Iva']:
                            ...
                            if not line_vals['value'].get('account_id', False):
                                line_vals['value']['account_id'] = self.pool['ir.property'].get(cr,uid,'property_account_income_categ', 'product.category').id
                            if not line_vals['value'].get('name', False):
                                line_vals['value']['name'] = _('AFIP declaration')
